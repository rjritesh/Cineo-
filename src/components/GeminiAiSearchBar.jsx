...
const GeminiAiSearchBar = () => {
  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-0">
      <div className="relative flex items-center bg-zinc-800 rounded-lg shadow-lg mx-auto w-full max-w-md sm:max-w-xl mb-8">
        <input
          ref={searchText}
          type="text"
          placeholder="Search for movie, series and more..."
          className="w-full py-3 sm:py-4 pl-10 sm:pl-12 pr-12 sm:pr-14 text-base sm:text-lg text-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
        />
        <button
          className="absolute right-2 sm:right-3 p-2 sm:p-3 text-white hover:bg-zinc-700 hover:rounded-full cursor-pointer focus:outline-none"
          onClick={handleGPTSearchClick}
        >
          <Search size={20} sm={24} />
        </button>
      </div>
      <GeminiAiSuggestion movies={aiSuggestedMovie} loading={loading} />
    </div>
  );
};
